...
def min_max(l: typing.Iterable):
    return min(l), max(l)


def max_minus_min(l: typing.Iterable):
    return max(l) - min(l)
...
def flatten(l: typing.Iterable) -> typing.List:
    "'Flattens' a list of lists into a single list. Example: [[1, 2], [3, 4]] -> [1, 2, 3, 4]"
    return [i for x in l for i in x]


def ints(s: str) -> typing.List[int]:
    "Returns a list of integers from a string. Example: '123 adslksdlk 456 789' -> [123, 456, 789]"
    return lmap(int, re.findall(r"-?\d+", s))  # thanks mserrano!


def positive_ints(s: str) -> typing.List[int]:
    """Returns a list of positive integers from a string. Example: '-123 adslksdlk 456 789' -> [456, 789]"""
    return lmap(int, re.findall(r"\d+", s))  # thanks mserrano!
...
def edit_distance(a, b) -> int:
    ...
    def aux(i, j):
        ...
        if i == n:
            ...
            dp[i][j] = min(
                (a[i] != b[j]) + aux(i + 1, j + 1), 1 + aux(i + 1, j), 1 + aux(i, j + 1)
            )
        return dp[i][j]
...
def knot(inp: str, binary: bool = False) -> str:
    ...
    return "".join(
        bin(i)[2:].zfill(8) if binary else hex(i)[2:].zfill(2) for i in sparse
    )
